...
def setup(args):
    ...
    platform_domain = input(f"Enter the platform domain (default: {args.platform_domain}): ").strip()
    keycloak_domain = input(f"Enter the keycloak domain (default: {args.keycloak_domain}): ").strip()
    ...
    if not os.path.exists('.env'):
        ...
        if not dry:
            ...
            # Replace domains
            replace_in_file('.env.temp', 'https://keycloak.bitswan.space', keycloak_domain)
            ...
            with open('.env.temp', 'a') as f:
                ...
                f.write(f"CCS_CONFIG_KEY={generate_secret()}\n")
            ...
            filter_vars('.env.temp', 'env/operations_centre-vars', '.operations_centre.env')

            # Update keycloak URL in operations centre env
            replace_in_file('.operations_centre.env', 'http.*keycloak:8080', keycloak_domain)

            filter_vars('.env.temp', 'env/influxdb-vars', '.influxdb.env')
            filter_vars('.env.temp', 'env/postgres-vars', '.postgres.env')
            filter_vars('.env.temp', 'env/discovery-vars', '.discovery_service.env')
        ...
        print("KEYCLOAK_CLIENT_SECRET - Instructions for getting this value here: "
        ...
        print("INFLUXDB_TOKEN - This can be gotten by going to the influxdb UI and going to data -> tokens -> generate")
        print("PORTAINER_ACCESS_TOKEN - This can be gotten by logging into the portainer UI, clicking on the username "
              "in the top right, then My account, then + add access token")
        print("If you are running locally you will also need to run the update-hosts command to update your hosts file:")
...
def update_hosts(args):
    ...
    if not dry:
        with open(hosts_file, 'a') as f:
            ...
            print("Hosts file has been updated. A backup was created at {hosts_file}.backup.*")
            print("Please try accessing your services again")


def main():
    parser = argparse.ArgumentParser(description='Automation Opererations Center management utility')
    subparsers = parser.add_subparsers(dest='command', help='Available commands')

    # Setup command
    setup_parser = subparsers.add_parser('setup', help='Setup environment configuration')
    setup_parser.add_argument('--platform-domain', default='http://localhost:3000',
                            help='Platform domain (default: %(default)s)')
    setup_parser.add_argument('--keycloak-domain', default='http://keycloak.platform.local',
                            help='Keycloak domain (default: %(default)s)')
    setup_parser.add_argument('--dry', action='store_true', help='Dry run')

    # Update hosts command
    hosts_parser = subparsers.add_parser('update-hosts', help='Update /etc/hosts file with required domains')
    hosts_parser.add_argument('--hosts-file', default='/etc/hosts', help='Path to hosts file (default: %(default)s)')
